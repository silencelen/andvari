...
Desktop), and a browser web app — sync through a central ktor server (CT 122 on
heimdall, Tailscale-only with a break-glass public tunnel). The server only ever
...
```sh
bash scripts/verify.sh   # gradle :core tests + web vitest, both off spec/test-vectors
```
