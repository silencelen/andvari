...
- X25519 via `crypto_box_seed_keypair(seed)`, `seed` = 32 random bytes generated by
  `tools/recovery-cli` on an **offline machine** — never the server, and never any
  always-on host that administers it.
...
- The private seed MUST NEVER exist in: git, any backup system, any object store or
  cloud, the server, or the disk of any always-on machine. recovery-cli holds it in
  memory only during a ceremony.
