...
- **F54** *(partial, low)* — the keepalive leg is FIXED (30s server ping shipped in v5, spec updated), and `notifyRevoked` now rings — five `App.kt` call sites since the M8 wave; remaining: no `{"type":"policy"}` frame producer anywhere in server main, while spec 03 §6 promises one and web `client.ts:546` handles a frame that never arrives. **Fix:** either add notifyPolicy or amend spec 03 §6.
