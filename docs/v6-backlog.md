...
> **SHIPPED IN TWO PASSES — read the item lines, not this banner.** Most of the batch went
> out 2026-07-08 (per §7.1 below; round-2 recon verified it live rather than re-finding it),
> but **F52 and F55 were still open under that banner for five weeks** and shipped
> **2026-08-13**. F28 and F54 remain **partial**, exactly as their own lines have always
> said. A blanket "DONE" over a per-item list is what let two open items read as closed;
> the per-item marks below are now the authority.
>
> *Re-verify before trusting F50's line: no global argon2 concurrency bound is visible in
> the tree today (`ServerCrypto.kt` has no `Semaphore`, and `login:` is still only the
> per-IP bucket in `App.kt`). Either it shipped by another mechanism or that leg is open.*
...
- **F51** ✅ **DONE 2026-07-27** (polish audit `quality-perf--1`) — `respondFileContent` now delegates to `respondFile` (streamed, contentLength set, never `file.readBytes()`), so installers no longer slurp into heap. *(was: yes, medium — `/downloads/{file}` slurped whole installers into heap (`App.kt:126-128`) while the attachment route right below already streamed.)*
- **F52** ✅ **DONE 2026-08-13** — shipped as scoped: `Notifier` owns a `SupervisorJob + Dispatchers.IO` scope, `notifyRev`/`notifyRevoked*` are no longer `suspend` (so no request path can await the bell), each delivery is one coroutine **per socket** bounded by `withTimeoutOrNull(SEND_TIMEOUT_MS = 2 s)`, and a send that does not land closes that socket instead of waiting for the 60 s pong reaper. Dropping a frame to a wedged peer is the correct trade — the bell is not a data plane, clients re-pull `/sync` on every (re)open (spec 03 §6). *(was: yes, medium — slightly worse post-Skipti: ~14 awaited call sites now — WS dirty-bell fanout awaited inline, sequential, no send timeout; one wedged socket stalls saves up to ~60s.)*
- **F55** ✅ **DONE 2026-08-13** (its last two legs; the rest landed in the 0.21.0 polish release) — `totp_setup` / `totp_confirm` / `totp_disable` each take a 5-per-minute per-user bucket, and a failed verify is now audited (`totp_verify_fail`, with a `where` tag and no PII) instead of only succeeding enrollments being visible; `login_fail` is audited for **unknown** emails too, so a stuffing run against addresses that don't exist here is no longer invisible — that row is deliberately anonymous (`userId` null, no meta) so it never names the submitted address. Earlier legs, already live: `bad_prefix` → 400 on the HIBP relay, `X-Content-Type-Options: nosniff` + `Referrer-Policy: no-referrer`, and RateLimiter window eviction with a hard cap. *(was: yes, medium — hardening pack: no TOTP confirm/disable limiter, HIBP bad prefix → 500 + stack trace, no nosniff/Referrer-Policy, RateLimiter windows never evict, login_fail audit only for known emails.)*
...
- **F54** *(partial, low)* — the keepalive leg is FIXED (30s server ping shipped in v5, spec updated), and `notifyRevoked` now rings — five `App.kt` call sites since the M8 wave; remaining: no `{"type":"policy"}` frame producer anywhere in server main. — **Corrected 2026-09-13 (audit H121):** the web handler described here **no longer exists** — `grep '"policy"' web/src/api/client.ts` is empty at HEAD; it was removed as dead code and this entry was never updated, so it credited the tree with half a feature it does not have. And the spec no longer promises the frame either: spec 03 §6 was amended in the same audit to list `rev` and `revoked` **exhaustively** and to record the policy frame as struck, not reserved. Nothing in the tree is now waiting on the other half. **Fix (if it is ever wanted):** it is a fresh design — a `notifyPolicy` producer, a client handler and a spec amendment, all three — not the one-line reconciliation this entry used to describe.
...
> deploy to the reference instance was pending owner go at the time of writing; **it went out
> long since — all five are fielded on every client.**

- **F60** ✅ **DONE 2026-07-09 (`7adecd7`, QW-2)** — the floor is `estimateStrength` score≥3, enforced at BOTH button-disable and submit on enrollment (Welcome) and change-password (Settings, including the forced-change path), with a live strength hint and the non-ASCII SHOULD-warn; shared helpers in `strength.ts` + `strength.test.ts`. *(was: yes, medium)* — master password floor is length≥8 while throwaway backup exports demand score≥3; spec 05 T8 claims enforcement that doesn't exist. **Fix:** `estimateStrength` score≥3 in Welcome enrollment + Settings change-password (covers the forced-change path), non-ASCII SHOULD-warn per spec 01 §1. *Do before real secrets migrate.*
- **F79** ✅ **DONE 2026-07-09 (`7adecd7`, QW-2)** — search matches notes + EVERY uri + a card's brand/••last4, on web, desktop AND Android under an identical predicate. Secrets are never search keys. *(was: yes, medium)* — search ignores notes and every URI after the first; desktop matches name+username only. **Fix:** two one-line predicate extensions (`Vault.tsx:169`, desktop `Ui.kt:199`); optionally mirror on Android.
- **F75** ✅ **CLOSED 2026-07-09 (no code change needed)** — the importers batch made the export panel's "re-import dedupes" claim TRUE, so the copy correction below was moot by the time QW-2 reached it. *(was: yes, medium)* — export panel promises re-import dedupe that doesn't exist (import panel says the opposite; `planImport` never consults the vault). **Fix now:** one-line copy correction in `ExportPanel.tsx:403` + test (error-truth doctrine). The vault-aware merge half folds into **importers** (see §3).
- **F76** ✅ **DONE 2026-07-09 (`7adecd7`, QW-2)** — `useBackGuard` steps Back through open layers (editor→import→export→detail→sub-view) instead of leaving the SPA. One armed sentinel at a time, self-consumed on in-app close AND on unmount — the review fix, because a lock/sign-out mid-layer would otherwise leak a Back-eating sentinel that accumulates. **Extended 2026-08-19 (`0ea5bd7`, shipped 0.24.0)**: the top-level views gained real hash-fragment addresses, built INTO the same guard (a second popstate consumer is forbidden). *(was: yes, medium)* — zero history-API usage; browser/gesture Back exits the SPA from anywhere, costing an argon2 unlock. **Fix:** sentinel `pushState` + popstate handler that steps view/detail/modal state back instead of leaving (~50 lines, not the full router the original M estimate assumed).
...
- **F64** ✅ **DONE 2026-07-09 (`7adecd7`, QW-2)** — the access/refresh TTLs are Admin fields, clamped ≥1 so a 0 cannot mint already-expired tokens. *(was: yes, medium value, trivially small)* — session access/refresh TTLs are live policy knobs with no Admin UI fields. **Fix:** two `numField` rows in `Admin.tsx`, clamp >0.
- **F78** ✅ **DONE 2026-07-09 (`7adecd7`, QW-2)** — the editor password is masked with a reveal toggle, and Generate over an existing value arms an inline confirm (house style, no native dialog) then auto-reveals the result. *(was: yes, low)* — editor password field is bare plaintext; Generate silently overwrites typed input. **Fix:** editable masked PasswordField + auto-reveal and confirm-overwrite on Generate; the length/class popover can wait.
...
- **F62** *(yes)* — `passwordHistory` and `favorite` are spec-normative and round-trip-protected on every layer but no client ever writes or displays them; silently losing the previous password on rotation is a mild data-loss hazard. **Fix:** shared append-retired-password helper in :core wired into all three editors + read-only reveal + favorite star; OR one-line spec/02 "reserved" marking. — **Decided 2026-08-13 (audit F29d): the `passwordHistory` half takes the "reserved" branch.** spec/02 §3 now marks the field reserved/planned, states that no v1 client writes it, and names item_versions (F63, shipped) as the *only* no-silent-loss backstop; spec/02 §7's "powers client-side password history repair" claim is struck. The rotation hazard itself is unchanged — building the helper stays open, it is just no longer a promise the normative spec makes on the product's behalf. — **Amended again 2026-09-13 (audit H121): "no client ever writes it" is now FALSE.** The 2026-08-18 spec/02 amendment gave the field exactly **one writer**, and it shipped: the Health *differs* resolution appends each losing duplicate's distinct password to the survivor's `passwordHistory` before the losers go to Trash, in both implementations (`web/src/ui/duplicates.ts`, `core/.../client/Duplicates.kt` — "this is `passwordHistory`'s first and only writer"). So a merge no longer destroys a password. What remains open is what this entry was really about: **no editor writes history on an ordinary rotation, and no client displays history at all**, so the reveal UI and the shared append-retired-password helper are still unbuilt. Read the entry as partially shipped, not open.
