...
3. **Recovery endpoints** (`/recovery/self/*`, `App.kt:723-752`) — now internet-reachable on single-origin instances — keep their unconditional per-IP 5/min and gain the same email-keyed backoff. **Build-time confirmation task (binding):** assert recovery-secret entropy meets target (≥128-bit) and the verify comparison is constant-time; document both in spec/05.
