 ...
 * are the CALLER's job. app-desktop carries an identical copy (the two app modules
...
/**
 * A confirmed backup preflight, pending its SAF destination pick. Held in
 * [AndvariViewModel] (NOT Compose `remember`) so a configuration change — Fold
 * unfold / rotation / split-screen resize — while the system picker is foreground
 * doesn't drop it and silently no-op the launcher result (leaving an empty file).
 * Holding the passphrase in the ViewModel is acceptable: the vault is unlocked, so
 * its keys are already in process memory; it is never written to SavedState/disk.
 */
